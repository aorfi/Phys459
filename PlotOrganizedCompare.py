...
for i in range(len(NListSR)):
    N = NListSR[i]
    M = N
    dataLocation = "Data/21-01-26/csN" + str(N) + "M" + str(M) + ".json"
    saved = []
    with open(dataLocation) as file:
        for line in file:
            saved.append(json.loads(line))
...
ax1.hist(engErrSRAll[index], bins=10, color = colors, label=labels)
ax1.set_xlabel("$\Delta E = |E_{RBM}-E_{ED}|$",size = 15)
ax2 = plt.subplot(gs[0, 1])
ax2.hist(stateErrSRAll[index], bins=10, color = colors, label=labels)
ax2.set_xlabel("$1-|<\Psi_{RBM}|\Psi_{ED}>|^2$",size = 15)

ax3 = plt.subplot(gs[0, 2])
ax3.hist(runTimeSRAll[index], bins=10, color = colors, label=labels)
...
# ax1 = plt.subplot(gs[0, 0])
# ax1.scatter(NListSR, avRunTimeSR)
